...
'''
SCRIPT CONTAINS ALL THE FUNCTIONS ASSOCIATED WITH EXTRACTING and PROCCESING REAL NARW DATA.

1. DO THE INITIAL TRIAGE: GOOD/BAD FILES, +ADJUST START/END POINTS on good ones ==> pickle
2. ADD SHALLOW/DEEP CLASSIFICATION ==> pickle
3. READ/STORE IN DICT GOOD SEGMENTS OF REAL DEPTH
4. CREATE STATE DP FROM REAL DP
5. OBTAIN VEC 0/1/2/3 OF LENGTH
6. OBTAIN NUMBER OF 2>1 AND 2>3 TRANSITIONS
7. PLOTS REAL DEPTH/STATE DEPTH/VEC0123 HISTO '''
...
def mark_bad_good_files(directory):
    ...
    pickle.dump(repertoire, open("repertoires/triage_repertoire3", "wb"))
...
def trimmed_selectedfiles(repertoire_to_improve,name_pickle_file):
    ...
    pickle.dump(new_repert, open("repertoires/"+name_pickle_file, "wb"))
    print('Pickle file /repertoires/' + name_pickle_file + 'created!')
...
def mark_shallow_deep_files(trimmed_repertoire_name):
    ...
    pickle.dump(trimmed_repertoire_class, open("repertoires/trimmed_repertoire_w_sdclass", "wb"))
    print('Pickle file repertoires/trimmed_repertoire_w_sdclass created!')
...
def extract_good_segment(repertoire_str):
    ...
    for i in range(len(repertoire)):
            ncin = Dataset('./DATA/NARWdive_profiles_data/' + repertoire[i][0], 'r', format='NETCDF')  # todo here issue
            print('./DATA/NARWdive_profiles_data/' + repertoire[i][0])
